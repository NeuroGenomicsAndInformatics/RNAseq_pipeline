#########################################################################
#Dockerfile for NGI RNA-seq pipeline -- editied from GTEx RNA-seq pipline
#########################################################################
FROM ubuntu:18.04
...
# bamsync -- not sure that I need this 
...
# salmon v1.7.0 (updated to 1.7.0 )
RUN cd /opt && \
	wget https://github.com/COMBINE-lab/salmon/releases/download/v1.7.0/salmon-1.7.0_linux_x86_64.tar.gz && \
	tar -xf salmon-1.7.0_linux_x86_64.tar.gz && rm salmon-1.7.0_linux_x86_64.tar.gz

ENV PATH /opt/salmon-latest_linux_x86_64:$PATH

# to prevent interactive part of installing R
...
#WORKDIR /usr/bin
RUN cp /src/linear_RNA_pipeline.py /usr/bin/rnaseq-linear
RUN cp /src/circ_RNA_pipeline.py /usr/bin/rnaseq-circ